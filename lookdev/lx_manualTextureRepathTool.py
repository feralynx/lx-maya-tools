# Manual Texture Repath Tool 2.1 - Alan Yang
...
def repathTextures(find_replace, dry_run, *args):
    ...
    for file in selFile:
        ...
        if files_UDIM:
            ...
            for tile in files_UDIM:
                ...
                if find_replace:
                    ...
                    elif len(tileName_noExtOrUDIM) == 1:
                        tileName_noExtOrUDIM = tileName_noExtOrUDIM[0]
                        tileName_udimAndExt = ''.join( ["." + s for s in tileName_split[-2:] ] )

                    elif not tileName_noExtOrUDIM:
                        tileName_noExtOrUDIM = tileName_split[:-1][0]
                        tileName_udimAndExt = ''.join( ["." + s for s in tileName_split[-1:] ] )
        ...
        if not files_UDIM:
            ...
            if copy: successfulFiles.append(fileName)
            else: failedFiles.append(fileName)
...
createUI( 'Manual Texture Repath Tool 2.1' )